...
def _fetch_benchmark_return(bm_ticker: str, from_date_str: str) -> float:
    """Percentage return of benchmark from from_date_str to today."""
    try:
...
def build_pnl_df(holdings: list[dict]) -> pd.DataFrame:
    ...
    for h in holdings:
        ...
        try:
            hold_days = (date.today() - datetime.strptime(bd, "%Y-%m-%d").date()).days
...
with st.sidebar:
    ...
    with st.expander("➕ 종목 추가", expanded=pm.count() == 0):
        with st.form("add_holding", clear_on_submit=True):
            ...
            qty_input  = col_q.number_input("수량", min_value=0.0, step=1.0)
            cost_input = col_c.number_input("평균매입가", min_value=0.0, step=100.0)
            buy_date_input = st.date_input("매입일", value=date.today())
            ...
            if group_sel == GROUP_ACCUMULATING:
                target_qty_input = st.number_input(
                    "목표 수량", min_value=0.0, step=1.0, value=0.0
                ) or None
    ...
    with st.expander("📂 CSV 업로드"):
        st.caption("컬럼: 종목코드, 수량, 평균매입가, 매입일 (+ 선택: 종목명, 그룹, 목표수량, 메모)")
...
with tab_dash:
    ...
    def _color_ret(val: float) -> str:
        return f"color:{UP};font-weight:bold" if val > 0 else (
               f"color:{DOWN};font-weight:bold" if val < 0 else "")
    ...
    valid_dates = [
        h.get("buy_date") for h in holdings_all
        if h.get("buy_date") and len(h["buy_date"]) == 10
    ]
    earliest = min(valid_dates) if valid_dates else str(date.today() - timedelta(days=365))
    ...
    bm_colors[0] = "#7E57C2"  # portfolio in purple to distinguish
    ...
    st.caption(f"기준일: {earliest} (포트폴리오 최초 매입일) → 현재")

    # ── Holding period scatter ────────────────────────────────────────────────
    df_hold = df_all.dropna(subset=["보유일"]).copy()
    if not df_hold.empty:
        st.divider()
        st.subheader("보유 기간 vs 수익률")
        fig_sc = px.scatter(
            df_hold, x="보유일", y="수익률(%)", text="종목",
            color="수익률(%)",
            color_continuous_scale=[[0, DOWN], [0.5, "#888"], [1, UP]],
            size=df_hold["평가금액"].clip(lower=1),
            size_max=40,
        )
        fig_sc.update_traces(textposition="top center", textfont_size=11)
        fig_sc.add_hline(y=0, line_dash="dot", line_color="rgba(128,128,128,0.4)")
...
with tab_acc:
    if df_acc.empty:
        ...
        st.caption("목표 수량까지 분할매수 중인 종목입니다. 달성 시 '보유 중'으로 이동하세요.")
        ...
        for _, row in df_acc.iterrows():
            ...
            with st.container(border=True):
                ...
                with c_info:
                    ...
                    col_m1.metric("수익률", f"{ret_pct:+.2f}%")
                    col_m2.metric("평가손익", f"{pnl:+,.0f}")
                    col_m3.metric("현재가", f"{row['현재가']:,.0f}")

                    if tgt and tgt > 0:
                        progress = min(qty / tgt, 1.0)
                        st.progress(progress, text=f"{qty:,.0f} / {tgt:,.0f} 주 ({progress*100:.1f}%)")
                    else:
                        st.caption(f"보유 수량: {qty:,.0f}주 (목표 미설정)")

                with c_btn:
                    st.markdown("<div style='margin-top:24px'></div>", unsafe_allow_html=True)
                    if st.button("완료\n→ 보유 중", key=f"move_{hid}", use_container_width=True):
        ...
        with st.expander("✏️ 수량/매입가 수정"):
            ...
            edit_sel_acc = st.selectbox("수정할 종목", list(edit_opts_acc.keys()), key="edit_acc_sel")
            edit_hid_acc = edit_opts_acc[edit_sel_acc]
            orig_acc = pm.get_by_id(edit_hid_acc)
            if orig_acc:
                ec1, ec2 = st.columns(2)
                new_qty_a  = ec1.number_input("수량",       value=float(orig_acc["quantity"]),  min_value=0.0, key="eqa")
                new_cost_a = ec2.number_input("평균매입가", value=float(orig_acc["avg_cost"]),   min_value=0.0, key="eca")
                new_tgt_a  = st.number_input("목표 수량",   value=float(orig_acc["target_qty"] or 0), min_value=0.0, key="eta")
                new_note_a = st.text_input("메모", value=orig_acc["notes"] or "", key="ena")
                if st.button("저장", key="save_acc", type="primary"):
                    pm.update_holding(
                        edit_hid_acc,
                        quantity=new_qty_a,
                        avg_cost=new_cost_a,
                        target_qty=new_tgt_a or None,
                        notes=new_note_a,
                    )
                    st.cache_data.clear()
                    st.success("수정 완료")
...
with tab_hld:
    if df_hld.empty:
        ...
        # Per-holding cards with analysis link
        for _, row in df_hld.iterrows():
            ...
            with st.container(border=True):
                ...
                with c_info:
                    ...
                    col_m1.metric("수량",     f"{row['수량']:,.0f}주")
                    col_m2.metric("평균매입가", f"{row['평균매입가']:,.0f}")
                    col_m3.metric("현재가",    f"{row['현재가']:,.0f}")
                    col_m4.metric("평가금액",  f"{row['평가금액']:,.0f}")
                    if row["매입일"]:
                        st.caption(f"매입일: {row['매입일']}  ·  보유 {row['보유일'] or '?'}일")

                with c_btns:
                    st.markdown("<div style='margin-top:8px'></div>", unsafe_allow_html=True)
                    if st.button("분석\n페이지", key=f"jump_hld_{hid}", use_container_width=True):
                        st.session_state["portfolio_jump_ticker"] = ticker
                        st.switch_page("pages/01_overview.py")
        ...
        with st.expander("✏️ 수량/매입가 수정"):
            ...
            if orig:
                ...
                new_bd   = st.text_input("매입일 (YYYY-MM-DD)", value=orig["buy_date"] or "", key="ebd")
                new_note = st.text_input("메모", value=orig["notes"] or "", key="en")
...
with tab_ai:
    ...
    st.caption(
        "전체 보유 종목 현황을 프롬프트로 자동 생성합니다. "
        "복사 후 Claude.ai에 붙여넣어 분석을 받으세요."
    )

    # Build portfolio summary table
    table_lines = ["| 종목 | 티커 | 그룹 | 수량 | 평단가 | 현재가 | 수익률 | 평가손익 |",
                   "|------|------|------|------|--------|--------|--------|---------|"]
    for _, row in df_all.iterrows():
        table_lines.append(
